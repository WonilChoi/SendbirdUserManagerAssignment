...
open class UserManagerBaseTests: XCTestCase {
    ...
    public let applicationId = Constants.appID   // Note: add an application ID
    public let apiToken = Constants.token        // Note: add an API Token
    
    // AppID1, AppID2로 동적으로 init해줬을 때, request error가 발생하므로
    // XCTAssertEqual(users.count, 1) > 무조건 users.count == 0이 발생할 수 밖에 없는 케이스입니다.
    // 임시로 주석처리했습니다.
